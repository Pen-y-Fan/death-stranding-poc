...
pub fn initialize() {
    #[cfg(target_arch = "wasm32")]
    {
        let deliveries: Vec<DemoDelivery> = match local_storage().get_item("deliveries").unwrap() {
            Some(value) => serde_json::from_str(&value).unwrap_or_else(|_| Vec::new()),
            None => Vec::new(),
        };
        let orders = vec![
            DemoOrder { id: 1, description: "Package A".to_string() },
            DemoOrder { id: 2, description: "Package B".to_string() },
            DemoOrder { id: 3, description: "Fragile Item".to_string() },
            DemoOrder { id: 4, description: "Important Document".to_string() },
            DemoOrder { id: 5, description: "Special Delivery".to_string() }
        ];
...
pub fn take_order(order_id: u32) {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        deliveries.push(DemoDelivery { order_id, status: "pending".to_string() });
        let _ = local_storage().set_item("deliveries", &serde_json::to_string(&deliveries).unwrap());
